...
const createDepartment = () => {
  inquirer
    ...
    .then(res => {
      ...
      pool.query('INSERT INTO departments (name) VALUES ($1)', [res.choice], (err: Error, result: QueryResult) => {
        if (err) {
          console.log(err);
        } else if (result) {
...
const createRole = () => {
  inquirer
    ...
    .then(res => {
      ...
      pool.query('INSERT INTO roles (title, salary, department) VALUES ($1, $2, $3)', [res.title, res.income, res.departmentId], (err: Error, result: QueryResult) => {
        if (err) {
          console.log(err);
        } else if (result) {
...
const updateEmployeeRole = () => {
  inquirer
    ...
    .then(res => {
      ...
      pool.query('UPDATE employees SET role_id = $1 WHERE id = $2', [res.roleId, res.employeeId], (err: Error, result: QueryResult) => {
        if (err) {
          console.log(err);
        } else if (result) {
...
const createEmployee = () => {
  inquirer
    ...
    .then(res => {
      ...
      pool.query('INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES ($1, $2, $3, $4)', [res.nameE, res.lastnameE, res.roleE, managerId], (err: Error, result: QueryResult) => {
        if (err) {
          console.log(err);
        } else if (result) {
